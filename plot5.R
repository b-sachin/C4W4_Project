...
neird <- subset(NEI, fips == "24510", c("SCC","Emissions","year"))

rd <- merge(neird,sccrd1,by="SCC")
...
g + geom_bar(stat = "identity")+  geom_text(aes(label = round(Emissions,0)), vjust = -0.2, size = 5,
  ...
  labs(title=expression("Total PM2.5 Emissions by Vehicals, Baltimore City  1999-2008"))
