...
sccrd0 <- grep("[Vv]ehicle(s)?",SCC$Short.Name, value = TRUE)

sccrd1 <- subset(SCC, Short.Name %in% sccrd0, SCC)
head(sccrd1)

both <- subset(NEI, fips == "24510" | fips == "06037", c("fips","SCC","Emissions","year"))

bothrd <- merge(both,sccrd1,by="SCC")

# Calculating total emission of pm2.5 for each year and type
rdbl <- aggregate(Emissions ~ year + fips, bothrd, sum)
head(rdbl)
...
g <- ggplot(rdbl, aes(year, Emissions, col=fips))

g + 
  geom_line() + 
  geom_point(size = 3) + 
  geom_text(aes(label = round(Emissions,0)), vjust = -0.7, size = 5, position = position_dodge(0.2)) + 
  scale_colour_discrete(name = "City", labels = c("Los Angeles", "Baltimore"))+ labs(x="Years", y="Total PM2.5 Emission (in Tons)") + 
  labs(title=expression("Total PM2.5 Emissions by Vehicals, Baltimore City and LA, 1999-2008"))
